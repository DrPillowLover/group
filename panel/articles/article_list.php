                        ...
                        <?php foreach ($articles as $article) : ?>
                            <tr>
                                ...
                                <td class="text-center align-middle">
                                    ...
                                    $sql = " SELECT * FROM `users` WHERE 'id' = 16 ";
                                    $stmt = $conn->query($sql)
                                    ?> </td>
<!--                                $sql = "select * from `users` where `id` = " . $article->user_id;-->
<!--                                $stmt = $conn->query($sql);-->
<!--                                $user = $stmt->fetch();-->
<!--                                echo $user->name;-->

                                <td class="text-center align-middle">
                                    <a class="btn btn-outline-info" href="<?= 'edit_category.php?id=' . $article->id ?>">
                                        ویرایش
                                    </a>
                                    <a class="btn btn-outline-danger"
                                       href="<?= 'delete_category.php?id=' . $article->id ?>">
                                        حذف
                                    </a>
                                </td>
                                <td class="text-center align-middle">
